...
import { EXTRAS_REGEX } from "./src/utils/mediaParser";
...
function inferCategory(baseDirName: string, fileName: string, extName: string, filePath?: string): string {
  ...
  // Check for extras/featurettes in the file path FIRST
  if (filePath) {
      const lowerFilePath = filePath.toLowerCase();
      if (EXTRAS_REGEX.test(lowerFilePath)) {
...
app.post("/api/scan", async (req, res) => {
  ...
  // Purge records that are no longer on disk for the paths we are scanning
  const validFilesSet = new Set(allFilesToScan.map(f => f.file.replace(/\\/g, '/')));
  const recordsToDelete: string[] = [];

  for (const rec of existingRecords) {
    if (!rec || !rec.filePath) continue;
    const recPath = rec.filePath.replace(/\\/g, '/');
    
    // Check if this record falls under any of the scanned paths
    const belongsToScannedPath = paths.some(scanPath => 
       recPath.toLowerCase().startsWith(scanPath.path.replace(/\\/g, '/').toLowerCase())
    );

    if (belongsToScannedPath && !validFilesSet.has(recPath)) {
       recordsToDelete.push(rec.id);
    }
  }

  if (recordsToDelete.length > 0 && db) {
    console.log(`[API] Purging ${recordsToDelete.length} obsolete records from database...`);
    res.write(JSON.stringify({ type: 'log', message: `Purging ${recordsToDelete.length} obsolete records from database...` }) + '\\n');
    ...
    for (let i = 0; i < recordsToDelete.length; i += chunkSize) {
       ...
       db.run(`DELETE FROM scanned_files WHERE id IN (${placeholders})`, chunk, (err: any) => {
         if (err) console.error("[DB] Error deleting obsolete records:", err);
       });
    }
  }
    ...
    while (pointer < allFilesToScan.length) {
      ...
      try {
        ...
        const parsedItem = {
          ...
          videoBitrate: (videoStream?.bit_rate || format.bit_rate) ? Math.round(parseInt(videoStream?.bit_rate || format.bit_rate) / 1000) : 0,
          videoBitrateMbps: (videoStream?.bit_rate || format.bit_rate) ? Math.round((parseInt(videoStream?.bit_rate || format.bit_rate) / 1000000) * 10) / 10 : 0,
          ...
          tags: format.tags || {},
          audioBitrate: audioStreams[0]?.bit_rate ? Math.round(parseInt(audioStreams[0].bit_rate) / 1000) : 0,
