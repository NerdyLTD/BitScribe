...
export function getReportTitle(scanType: string): string {
  if (scanType === "Discovery Scan") return "BitScribe - Discovery Audit";
  if (scanType === "Stream Audit Scan") return "BitScribe - Streaming Audit";
  if (scanType === "Subtitle Scan") return "BitScribe - Subtitle Audit";
  if (scanType === "Metadata Scan" || scanType === "Video Metadata Scan" || scanType === "Music Metadata Scan") return "BitScribe - Metadata Audit";
  if (scanType === "Duplication Scan") return "BitScribe - Duplicate Files Audit";
  if (scanType === "Corrupted Audit") return "BitScribe - Bad Files Audit";
  if (scanType === "Anomaly Scan") return "BitScribe - Quality Audit";
  return "BitScribe - Media Library Audit";
}
...
export async function exportMediaLibraryToCSV(
  ...
  const allPossibleHeaders = [
    ...
    "Audio Codecs",
    "Subtitles",
    ...(rules.useSubtitleScan ? ["Subtitle Technical Data"] : []),
    "Series Title",
    "Season",
    "Episode",
    "Episode Title",
  ...
  const rows = items.map((item) => {
    ...
    const data: Record<string, any> = {
      ...
      Subtitles: subStr,
      "Subtitle Technical Data": rules.useSubtitleScan ? formatSubtitleTechnical(item) : "",
      "Series Title":
...
export async function exportMediaLibraryToHTML(
  ...
  let allPossibleHeaders = [
    ...
    "Audio Codecs",
    "Subtitles",
    ...(isSubtitle ? ["Subtitle Technical Data"] : []),
    "Series Title",
    "Season",
    "Episode",
    "Episode Title",
  ...
  if (isDuplication) {
    ...
    allPossibleHeaders = ["File", "Path", "Duplicate File", "Duplicate Path"];
    finalColumns = { "File": true, "Path": true, "Duplicate File": true, "Duplicate Path": true };
  } else {
    optimizedItems = targetItems.map(item => {
      ...
      if (isSubtitle) {
        rowData["Subtitle Technical Data"] = formatSubtitleTechnical(item);
      }
...
<script>
  ...
  const getColsForCat = (cat) => {
    const isVideo = ["Movies", "Movie", "Documentaries", "TV Shows", "TV", "Docuseries", "Documentary Series", "Extras", "Shorts", "Plays", "Specials"].includes(cat);
    const isMusic = ["Music Albums", "Soundtracks", "Music Compilations", "Music"].includes(cat);
    
    if (isDiscovery) {
      if (cat === "TV Shows" || cat === "TV") return ["Series Title", "Season", "Episode", "Episode Title", "Video Codec", "Resolution", "Audio Codecs", "Audio Tracks", "Release Year", "File Path"];
    ...
    if (isMetadata) {
      return ["File Name", "Missing Metadata", isMusic ? "Cover Art" : "Poster", "File Path"];
    }
    ...
    if (isSubtitle) {
      return ["Alert Level", "File Name", "Subtitles", "Subtitle Technical Data", "Analysis Notes", "Remediation Action", "File Path"];
    }
  ...
  function initCategories() {
    ...
    const order = ["Movies", "Movie", "Documentaries", "Docuseries", "Documentary Series", "TV Shows", "TV", "Plays", "Specials", "Extras", "Shorts", "Music Albums", "Soundtracks", "Music Compilations", "Music", "Other", "Static", "Corrupted"];
  ...
  function renderTable() {
    ...
    document.getElementById('table-head').innerHTML = '<tr>' + 
      activeHeaders.map(h => {
        ...
        return '<th onclick="setSort(&quot;'+h.replace(/"/g, '&quot;')+'&quot;)" data-col="'+h.replace(/"/g, '&quot;')+'" '+widthStyle+' class="px-4 py-3 text-xs font-semibold tracking-wider text-slate-400 uppercase border-b border-slate-700 text-center align-top select-none relative '+sc+'">'+displayLabel+'</th>';
      }).join('') + '</tr>';
