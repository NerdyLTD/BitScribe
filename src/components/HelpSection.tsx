...
import { downloadUnifiedPresets, downloadPresetsZip, PRESET_BLUEPRINTS } from '../utils/handbrakePresets';
...
export default function HelpSection({ highlightId, isTourActive, tourStepIndex, activeDemo }: HelpSectionProps) {
  ...
  // HandBrake Preset Explorer States
  const [activeStandard, setActiveStandard] = useState<'Modern+' | 'Legacy+'>('Modern+');
  const [activeQuality, setActiveQuality] = useState<'Space Saver' | 'Balanced' | 'Kinda Silly'>('Space Saver');
