...
content = re.sub(r'function getTopLevelFolder.*?return parts\.length > 1 \? parts\[0\] : "Root";\n\}', new_functions.strip(), content, flags=re.DOTALL)

content = content.replace("const base = f.replace(/\\\\/g, '/').split('/').pop();", "const base = fastBasename(f);")
content = content.replace('const gfBasename = gf.filename || gf.filePath.replace(/\\\\/g, \'/\').split(\'/\').pop() || "";', 'const gfBasename = gf.filename || fastBasename(gf.filePath) || "";')
content = content.replace('const ghostBasenames = new Set(ghostFiles.map(gf => (gf.filename || gf.filePath.replace(/\\\\/g, \'/\').split(\'/\').pop() || "").toLowerCase()));', 'const ghostBasenames = new Set(ghostFiles.map(gf => (gf.filename || fastBasename(gf.filePath) || "").toLowerCase()));')
content = content.replace('const nfBasename = nf.replace(/\\\\/g, \'/\').split(\'/\').pop() || "";', 'const nfBasename = fastBasename(nf) || "";')
