## [Unreleased]
- **macOS Bundle Sealing & Gatekeeper Corruption Fix**: Fixed `resolve_data_dir()` in `apps/steward/src-tauri/src/lib.rs` to detect when the executable is running inside an `.app` bundle (`is_inside_app_bundle`). Previously, running the app outside `/Applications` (e.g. from `Downloads` or mounted `.dmg`) caused the app to treat its internal `.app/Contents/MacOS/` folder as a writable directory, writing SQLite databases (`steward.db`), log files, and native binaries directly inside the bundle. This runtime bundle modification invalidated the macOS code signature seal, causing macOS Gatekeeper to report "app is damaged and cannot be opened". Data paths are now strictly routed to `~/Library/Application Support/BitScribeSteward/` across all macOS execution modes.
