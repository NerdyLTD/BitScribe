## [Unreleased]
- **macOS Apple Silicon Release & Gatekeeper Optimization**: Updated `apps/steward/src-tauri/tauri.conf.json` with `bundle.macOS.minimumSystemVersion: "11.0"` and configured `MACOSX_DEPLOYMENT_TARGET: "11.0"` in `.github/workflows/release.yml`. Added explicit macOS Gatekeeper instruction (`xattr -cr`) to release notes to bypass Apple Silicon's un-notarized app security flag ("damaged package" warning).
- **BitScribe RX File Renamer Architecture Specification**: Authored `Suite/BitScribeRX/RENAMER_SPECIFICATION.md` detailing preset categories, token schema (`{Title}`, `{Initials}`, `{Codec}`, `{Res}`, etc.), multi-modal UI selection hierarchy (file/directory/logical), native TS/Rust execution engine, and safety safeguards (pre-flight collision checks, atomic case-only renaming, transaction history undo, sidecar tracking).
- **GitHub Release Workflow Optimization**: Added `projectPath: apps/steward` to `tauri-action` in `.github/workflows/release.yml` to ensure CI builds locate `src-tauri` in the monorepo workspace properly. Verified M1 Mac (Apple Silicon `aarch64-apple-darwin`) release bundle settings and sidecar binary permissions.
- **Tauri Backend Clean-up**: Prefixed unused `app` parameter in `setup_ffprobe` (`_app`) in `apps/steward/src-tauri/src/lib.rs` to clean up Rust compiler dev warnings.
- **Workspace Type Safety & Code Quality Audit**: Conducted thorough file-by-file code review across all packages and apps (`@bitscribe/core-types`, `@bitscribe/core-eval`, `@bitscribe/core-db`, `@bitscribe/core-export`, `@bitscribe/ui-components`, and `steward`). Resolved all strict TypeScript errors, implicit `any` parameter types, missing null/undefined guards, and state callback signatures (`tsc --noEmit` and `vite build` 100% clean).
...
- **Diagnostic Logging Enhancements**: Upgraded the internal diagnostic logging system to automatically capture and append high-resolution performance profiling (via `performance.now()`) across the primary orchestrator loops. Included a trace system to capture the 10 slowest files scanned during FFprobe execution, alongside a static architecture, dependency, and SQLite schema reference dump at the head of every log file to assist in automated troubleshooting and LLM diagnostics.
- **Scan Time Persistence Fix**: Resolved an issue where the displayed scan time could revert to an older value (such as a previous scan's duration) if the app was minimized or left idle for a while on Windows. The app now properly synchronizes settings and scan durations back to the native backend in real-time, ensuring that if the system suspends or refreshes the background webview, the latest correct scan time is restored.
- **Empty Report Generation**: Fixed an issue in the report exporter where reports (such as the Quality Audit) would be silently skipped and not exported at all if the scan yielded zero matching results. The exporter will now generate and save the report with headers and a clean sheet, assuring the user that the audit ran successfully but found zero anomalies.
...
### UI Responsiveness Enhancements (Scan & Library Tabs)
