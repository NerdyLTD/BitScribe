## [Unreleased]
- **macOS Apple Silicon Release & Packaging Fix**: Configured `hardenedRuntime: false` and `signingIdentity: "-"` (ad-hoc signing) in `apps/steward/src-tauri/tauri.conf.json`. Disabling Hardened Runtime for un-notarized ad-hoc builds prevents macOS Gatekeeper on Apple Silicon from rejecting the binary and throwing the "damaged app" error upon launch.
