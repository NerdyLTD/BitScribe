## [Unreleased]
- **macOS Bundle Signing Restoration**: Reverted ad-hoc code signing () back to default bundle configuration in . Forcing explicit ad-hoc signing () without Developer ID entitlements caused macOS Gatekeeper on Apple Silicon to corrupt bundle identity and silently abort WebKit window creation on startup.
- **Node.js 22 Release Pipeline Lock**: Explicitly locked Node.js version to  in  to prevent runner version drift across release builds.
