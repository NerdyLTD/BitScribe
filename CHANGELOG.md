## [Unreleased]
- **macOS GUI Opening & Apple Notarization Pipeline**: Updated `.github/workflows/release.yml` with Apple Developer ID signing secrets (`APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_PASSWORD`, `APPLE_TEAM_ID`) to support 1-click notarized release builds. Updated release notes with 100% GUI launch instructions (Right-Click -> Open in Finder) requiring no command line usage.
