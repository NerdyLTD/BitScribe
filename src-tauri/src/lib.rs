...
#[tauri::command]
async fn clear_db(state: State<'_, DbState>) -> Result<(), String> {
    let conn = state.conn.lock().unwrap();
...
#[tauri::command]
async fn get_diagnostic() -> serde_json::Value {
...
#[tauri::command]
async fn save_file(path: String, contents_b64: String) -> Result<(), String> {
